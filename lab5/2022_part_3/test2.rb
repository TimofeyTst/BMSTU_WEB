...
# Class for test Solution from main
...
# Class for test Solution from main2
RSpec.describe Solution2 do
  describe '#main2' do
    it 'should return not changed strings' do
      test_strings = ['Ты красивая как осень хочу тебя целовать', 'В твоей голове вопросы В моей просто пустота']
      correct_strings = Solution2.new(test_strings).make_correct
      expect(correct_strings).to eq(test_strings)
    end

    it 'should return with deleting strings contains only spec symbols' do
      test_strings = ['Ты красивая как осень хочу тебя целовать', 'В твоей голове вопросы В моей просто пустота']
      correct_strings = Solution2.new(test_strings + ['&hasd', '*dasf', 'asd)']).make_correct
      expect(correct_strings).to eq(test_strings)
    end
    ...
    it 'should returns true for auto generating strings' do
      ...
      5000.times do
        ...
        expect(Solution2.new(uncorrect_strings).make_correct).to eq(correct_strings)
        # ind += 1
        # if (ind == 5)
        #     expect(Solution2.new(uncorrect_strings).make_correct).to eq(correct_strings + ["test"])
        # end
      end
    end
  end
